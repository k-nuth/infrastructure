 ...
 */
#ifndef LIBBITCOIN_INFRASTUCTURE_WALLET_MESSAGE_HPP
#define LIBBITCOIN_INFRASTUCTURE_WALLET_MESSAGE_HPP
 ...
 */
static BI_CONSTEXPR size_t message_signature_size = 1 + ec_signature_size;
typedef byte_array<message_signature_size> message_signature;

/**
 * Hashes a messages in preparation for signing.
 */
BI_API hash_digest hash_message(data_slice message);
 ...
 */
BI_API bool sign_message(message_signature& signature, data_slice message,
 ...
 * @return true if wif is valid and signature encoding is successful.
 */
BI_API bool sign_message(message_signature& out_signature, data_slice message,
 ...
 */
BI_API bool verify_message(data_slice message, const payment_address& address,
    const message_signature& signature);

/// Exposed primarily for independent testability.
BI_API bool recovery_id_to_magic(uint8_t& out_magic, uint8_t recovery_id,
    bool compressed);

/// Exposed primarily for independent testability.
BI_API bool magic_to_recovery_id(uint8_t& out_recovery_id, bool& out_compressed,
    uint8_t magic);
